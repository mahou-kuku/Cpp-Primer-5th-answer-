...
* float 和 double 的区别： 这两种类型都是浮点数类型，用于表示实数。它们的区别在于所能表示的数值范围和精度。float 通常有 32 位，而 double 通常有 64 位。由于 double 类型具有更多的位宽，因此它能表示的数值范围更大，精度也更高。然而，尽管 double 的精度更高，但其计算代价与 float 相差无几，在某些机器上，双精度运算甚至比单精度运算还快。因此，在实际应用中通常选择 double 类型。  
